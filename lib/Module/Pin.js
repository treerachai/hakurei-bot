...
class Pin {
    constructor(services, options) {
        ...
        services.discord.on('messageCreate', this.onMessage, this);
        services.discord.on('messageUpdate', this.onMessageUpdate, this);
        services.events.on('command.confirm', this.onConfirm, this);
    }
    ...
    onMessage(message) {
        ...
        if (this.isPinnable(message)) {
            ...
            let reaction = message.reactions[this.options[message.channel.id]];
            if (!reaction || !reaction.me) {
                message.addReaction(this.options[message.channel.id].emojiName).catch(() => {});
            }
        }
    }
            ...
            if (this.pins[channel.id].length < this.options[channel.id].limit ||
                ...
                messages.forEach((message) => {
                    ...
                    if ((!reaction || !reaction.me) && this.isPinnable(message)) {
                        message.addReaction(this.options[channel.id].emojiName).catch(() => {});
                    }
                });
            }

            if (messages.length == 100 && left > 100) {
                setTimeout(() => this.parseMessages(channel, left - messages.length, messages[messages.length - 1].id));
            }
        });
    }
     ...
     */
    pinMessage(message) {
        console.log('Pinning: ' + (message.embeds.length > 0 ? message.embeds[0] && message.embeds[0].url : message.attachments[0] && message.attachments[0].url));
        message.pin();
    ...
    unpinMessage(message, collection) {
        console.log('Unpinning: ' + (message.embeds.length > 0 ? message.embeds[0] && message.embeds[0].url : message.attachments[0] && message.attachments[0].url));
        collection.splice(collection.indexOf(message), 1);

        if (message.embeds.length > 0){
            message.channel.createMessage('Removido: <' + (message.embeds[0] && message.embeds[0].url) + '>');
        }
        if (message.attachments.length > 0){
            message.channel.createMessage('Removido: <' + (message.attachments[0] && message.attachments[0].url) + '>');
        }
