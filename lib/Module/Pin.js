...
class Pin {
    ...
    onReaction(event) {
        ...
        if (event.emoji.name != options.pin[event.channel.id].emojiName) return; // Ignore other emojis

        if (event.message) {
            let reaction = event.message.reactions.find((reaction) => {
                return reaction.emoji.name == options.pin[event.channel.id].emojiName;
            })
    ...
    onMessage(event) {
        ...
        if (this.isPinnable(event.message)) {
            ...
            if (event.message.reactions.length == 0) {
                event.message.addReaction(options.pin[event.message.channel.id].emojiName);
            }
        }
    }
            ...
            if (collection.length < options.pin[channel.id].limit ||
                ...
                data.messages.forEach((message) => {
                    let reaction = message.reactions.find((reaction) => {
                        return reaction.emoji.name == options.pin[channel.id].emojiName;
                    });
                    ...
                    if ((!reaction || !reaction.me) && this.isPinnable(message)) {
                        message.addReaction(options.pin[channel.id].emojiName);
                    }
                });
            }
