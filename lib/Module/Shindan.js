...
class Shindan {
    constructor(services, options) {
        ...
        this.events.on('command.shindan', (i, m, a) => { this.onShindan(i, m, a); });
        this.events.on('command.shindan.delete', (i) => { this.onDelete(i); });
    }
    ...
    onShindan(id, message, argv) {
        ...
        if (argv[1] && /^\d+$/.test(argv[1])) {
            ...
            let name = message.author.name || message.author.username;
            request.post({
                ...
                message.channel.sendMessage(this.currentShindan, false, embed).then((message) => {
